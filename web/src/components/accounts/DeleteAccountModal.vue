...
import { Trash2, RefreshCw, ShieldAlert } from '@lucide/vue'
import {
  Dialog,
  DialogContent,
  DialogFooter,
...
<template>
  <Dialog :open="open && !!account" @update:open="(val) => { if (!val) emit('close') }">
    <DialogContent v-if="account" class="max-w-md p-0 overflow-hidden border-[#2c2e36] bg-[#1a1c22]">
      <!-- Header -->
      <div class="px-6 py-4 border-b border-[#2c2e36] flex items-center gap-3 bg-[#141518]">
        ...
        <div>
          <DialogTitle class="text-sm font-semibold text-white">Remove Account</DialogTitle>
          <DialogDescription class="text-xs text-zinc-400 mt-0.5">Disconnect account from proxy pool</DialogDescription>
        </div>
      </div>
      ...
      <DialogFooter class="px-6 py-3.5 border-t border-[#2c2e36] bg-[#141518] flex items-center justify-end gap-2.5">
        ...
        >
          ...
          <span>{{ busy ? 'Removing...' : 'Remove Account' }}</span>
        </Button>
      </DialogFooter>
    </DialogContent>
  </Dialog>
