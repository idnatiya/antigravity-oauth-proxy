...
// GetThoughtSignature returns the thought signature.
func (p *ContentPart) GetThoughtSignature() string {
	return p.ThoughtSignature
}

// SetThoughtSignature sets the thought signature on the content part.
...
func (p *ContentPart) UnmarshalJSON(b []byte) error {
	...
	*p = ContentPart(a.alias)
	if p.ThoughtSignature == "" && a.ThoughtSignatureSnake != "" {
		p.ThoughtSignature = a.ThoughtSignatureSnake
	}
	return nil
}
